...
let previousOperation = document.querySelector(
  ".previous-operation"
) as HTMLButtonElement;
let currentOperation = document.querySelector(
  ".current-operation"
) as HTMLButtonElement;
  ...
  try {
    if (!isNaN(Number(valueForLogybaseFunctions))) {
      ...
      currentOperation.value = (
        Math.log(Number(valueForLogybaseFunctions)) /
        Math.log(Number(currentOperation.value))
      ).toString();
    ...
    if (!isNaN(Number(valueForNthrootFunctions))) {
      ...
      currentOperation.value = Math.pow(
        Number(valueForNthrootFunctions),
        1 / Number(currentOperation.value)
      ).toString();
...
let appendInput = function (event: Event) {
  ...
  if (
    ...
    currentOperation.value = (event.target as HTMLDivElement).textContent!;
  } else {
    currentOperation.value += (event.target as HTMLDivElement).textContent!;
  }
};
let appendOperator = function (event: Event) {
  if (!currentOperation.value.length) {
    return;
  }
  if (
...
let parenthesisOpen = function (event: Event) {
  if (!currentOperation.value.length) {
    return;
  }
  if (
...
let parenthesisClose = function (event: Event) {
  if (!currentOperation.value.length) {
    return;
  }
  if (
